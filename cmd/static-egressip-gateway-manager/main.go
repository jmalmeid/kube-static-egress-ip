...
func main() {
	...
	glog.Infof("Running egress ip gateway manager version: " + version.Version)
	...
	gatewayManager := ha.NewGatewayManager(kubeClient, egressipClient,
		egressipInformerFactory.Staticegressips().V1alpha1().StaticEgressIPs().Lister())

	egressipInformerFactory.Start(stopCh)

	if err = gatewayManager.Run(stopCh); err != nil {
